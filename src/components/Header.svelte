...
<nav class={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-black/90 py-3 backdrop-blur-md border-b border-white/10' : 'bg-transparent py-5'}`}>
  <div class="container mx-auto px-6 flex justify-between items-center">
    <div class="flex items-center gap-2">
      <div class="w-10 h-10 bg-red-600 flex items-center justify-center rounded-lg shadow-lg shadow-red-600/20">
        <i class="fas fa-bolt text-white text-xl"></i>
      </div>
      <span class="text-2xl font-oswald font-bold tracking-tighter text-white uppercase italic">
  ...
  <!-- Mobile Menu -->
  <div class={`md:hidden absolute w-full bg-black border-b border-white/10 transition-all duration-300 overflow-hidden ${mobileMenuOpen ? 'max-h-96 py-6' : 'max-h-0'}`}>
