<script lang="ts">
  ...
  function openLightbox(index: number) {
    currentImage = filteredImages.findIndex(img => galleryImages.indexOf(img) === index)
    lightboxOpen = true
  }
...
<!-- Lightbox Modal -->
{#if lightboxOpen}
  <div class="fixed inset-0 z-50 bg-black/95 flex items-center justify-center p-4" on:click={closeLightbox}>
    <button
    ...
    <div class="max-w-6xl w-full" on:click|stopPropagation>
      ...
      />
      <p class="text-center text-white mt-4 font-bold uppercase tracking-widest">
